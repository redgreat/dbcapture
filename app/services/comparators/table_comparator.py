...
class TableComparator(BaseComparator):
    ...
    def __init__(self, db):
        super().__init__(db)
        self.comparison_type = ComparisonType.TABLE
        
    def _get_tables(self, conn: pymysql.Connection) -> Dict[str, str]:
        """获取数据库中的所有表及其创建语句"""
        tables = {}
    ...
    def _get_table_columns(self, conn: pymysql.Connection, table_name: str) -> Dict[str, Dict]:
        """获取表的列定义"""
        columns = {}
        with conn.cursor() as cursor:
    ...
    def _get_table_indexes(self, conn: pymysql.Connection, table_name: str) -> Dict[str, Dict]:
        """获取表的索引定义"""
        indexes = {}
        with conn.cursor() as cursor:
    ...
    def _get_table_constraints(self, conn: pymysql.Connection, table_name: str) -> Dict[str, Dict]:
        """获取表的约束定义"""
        constraints = {}
        with conn.cursor() as cursor:
            # 获取外键约束
    ...
    def _compare_table_details(
        ...
        # 比较列
        column_diffs = self._compare_columns(source_columns, target_columns)
        if column_diffs:
            differences["columns"] = column_diffs
            
        # 比较索引
        index_diffs = self._compare_indexes(source_indexes, target_indexes)
        if index_diffs:
            differences["indexes"] = index_diffs
            
        # 比较约束
        constraint_diffs = self._compare_constraints(source_constraints, target_constraints)
        if constraint_diffs:
            differences["constraints"] = constraint_diffs
            
        return differences

    def _compare_columns(self, source: Dict[str, Dict], target: Dict[str, Dict]) -> Dict[str, Any]:
        """比较列定义"""
        differences = {}
        ...
        for col in all_columns:
            if col not in source:
                differences[col] = {"type": "missing_in_source", "message": f"列 {col} 在源表中不存在"}
            elif col not in target:
                differences[col] = {"type": "missing_in_target", "message": f"列 {col} 在目标表中不存在"}
            else:
    ...
    def _compare_indexes(self, source: Dict[str, Dict], target: Dict[str, Dict]) -> Dict[str, Any]:
        """比较索引定义"""
        differences = {}
        ...
        for idx in all_indexes:
            if idx not in source:
                differences[idx] = {"type": "missing_in_source", "message": f"索引 {idx} 在源表中不存在"}
            elif idx not in target:
                differences[idx] = {"type": "missing_in_target", "message": f"索引 {idx} 在目标表中不存在"}
            else:
    ...
    def _compare_constraints(self, source: Dict[str, Dict], target: Dict[str, Dict]) -> Dict[str, Any]:
        """比较约束定义"""
        differences = {}
        ...
        for constr in all_constraints:
            if constr not in source:
                differences[constr] = {"type": "missing_in_source", "message": f"约束 {constr} 在源表中不存在"}
            elif constr not in target:
                differences[constr] = {"type": "missing_in_target", "message": f"约束 {constr} 在目标表中不存在"}
            else:
                ...
                # 只在外键约束时比较引用信息
                if source_constr["type"] == "FOREIGN KEY" and target_constr["type"] == "FOREIGN KEY":
                    if source_constr.get("referenced_table") != target_constr.get("referenced_table"):
                        constr_diffs["referenced_table"] = {
                            "source": source_constr.get("referenced_table"),
                            "target": target_constr.get("referenced_table")
                        }
                    if set(source_constr.get("referenced_columns", [])) != set(target_constr.get("referenced_columns", [])):
    ...
    def _generate_table_change_sql(self, table_name: str, differences: Dict[str, Any]) -> str:
        """生成表结构变更SQL"""
        sql_statements = []
                ...
                for constr_name, constr_diff in differences["constraints"].items():                   
                    if constr_diff.get("type") == "missing_in_target":
                        ...
                        if source_constr["type"] == "FOREIGN KEY":
                            # 检查必要字段
                            if not all(key in source_constr for key in ["columns", "referenced_table", "referenced_columns"]):
                                continue
    ...
    def _do_compare(self, comparison_id: int, source_conn: pymysql.Connection, target_conn: pymysql.Connection) -> List[ComparisonResult]:
        ...
        for table_name in all_tables:
            ...
            source_indexes = self._get_table_indexes(source_conn, table_name)
            target_indexes = self._get_table_indexes(target_conn, table_name)
            
            source_constraints = self._get_table_constraints(source_conn, table_name)
            target_constraints = self._get_table_constraints(target_conn, table_name)
            ...
            if differences:
                result = self._create_result(
                    ...
                    change_sql=self._generate_table_change_sql(table_name, differences)
                )
                results.append(result)
            else:
                result = self._create_result(

...
class ViewComparator(BaseComparator):
    ...
    def __init__(self, db):
        super().__init__(db)
        self.comparison_type = ComparisonType.VIEW
        
    def _get_views(self, conn: pymysql.Connection) -> Dict[str, str]:
        """获取数据库中的所有视图及其创建语句"""
        views = {}
    ...
    def _do_compare(self, comparison_id: int, source_conn: pymysql.Connection, target_conn: pymysql.Connection) -> List[ComparisonResult]:
        """执行视图比较"""
        # 获取源数据库和目标数据库中的所有视图
        source_views = self._get_views(source_conn)
        target_views = self._get_views(target_conn)

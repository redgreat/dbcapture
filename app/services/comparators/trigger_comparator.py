...
class TriggerComparator(BaseComparator):
    ...
    def __init__(self, db):
        super().__init__(db)
        self.comparison_type = ComparisonType.TRIGGER
        
    def _get_triggers(self, conn: pymysql.Connection) -> Dict[str, str]:
        """获取数据库中的所有触发器及其创建语句"""
        triggers = {}
    ...
    def _do_compare(self, comparison_id: int, source_conn: pymysql.Connection, target_conn: pymysql.Connection) -> List[ComparisonResult]:
        """执行触发器比较"""
        # 获取源数据库和目标数据库的所有触发器
        source_triggers = self._get_triggers(source_conn)
        target_triggers = self._get_triggers(target_conn)

...
class FunctionComparator(BaseComparator):
    ...
    def __init__(self, db):
        super().__init__(db)
        self.comparison_type = ComparisonType.FUNCTION
        
    def _get_functions(self, conn: pymysql.Connection) -> Dict[str, str]:
        """获取数据库中的所有函数及其创建语句"""
        functions = {}
    ...
    def _do_compare(self, comparison_id: int, source_conn: pymysql.Connection, target_conn: pymysql.Connection) -> List[ComparisonResult]:
        """执行函数比较"""
        # 获取源数据库和目标数据库的所有函数
        source_functions = self._get_functions(source_conn)
        target_functions = self._get_functions(target_conn)
